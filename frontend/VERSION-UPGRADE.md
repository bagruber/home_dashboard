...
| `typescript` | ^5.6.2 | ~5.9.x | Welle 1 — nur Typen, kein Laufzeitverhalten |

## Besonderheit

**Der aufwendigste Fall.** Tailwind 3 -> 4 ist keine Versionserhoehung, sondern ein Konfigurations-Wechsel: `tailwind.config.js` entfaellt zugunsten CSS-first-Konfiguration, `postcss`/`autoprefixer` werden nicht mehr gebraucht. Zuerst Tailwind separat migrieren, dann der Rest. Betrifft nur `frontend/`; das Python-Backend bleibt unberuehrt.
