...
| `tailwindcss` | ^3.4.13 | ^4.x | Welle 3 — Runtime-Major |
| `vite` | ^5.4.8 | ^8.x | Welle 2 — Build-Tool-Major |
| `typescript` | ^5.6.2 | ~7.0.2 | Welle 3 — Runtime-Major |

## Besonderheit

**Der aufwendigste Fall der Welle 3.** Tailwind 3 -> 4 ist keine Versionserhoehung, sondern ein Konfigurations-Wechsel: `tailwind.config.js` entfaellt zugunsten CSS-first-Konfiguration, `postcss` und `autoprefixer` werden nicht mehr gebraucht. Zuerst Tailwind allein migrieren, dann der Rest. Betrifft nur `frontend/`; das FastAPI-Backend bleibt unberuehrt. `react-resizable` wurde am 26.08.2026 als direkte Dependency ergaenzt — nicht wieder entfernen.
